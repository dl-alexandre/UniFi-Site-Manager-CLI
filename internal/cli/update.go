...
import (
	...
	"github.com/dl-alexandre/UniFi-Site-Manager-CLI/internal/cache"
...
// Run executes the update check - accepts a Formatter for output
func (c *CheckUpdateCmd) Run(formatter *output.Formatter) error {
	// Get current version
	...
	var cacheInstance *cache.SimpleCache
	if cacheDir != "" {
		cacheInstance = cache.NewSimpleCache(cacheDir)
	}

	// Try to get from cache first
	if !c.Force && cacheInstance != nil {
		if cached, ok := cacheInstance.Get(updateCacheKey); ok {
			if info, valid := cached.(UpdateInfo); valid {
				return c.displayUpdateInfo(&info, formatter)
			}
		}
	}
	...
	if cacheInstance != nil {
		cacheInstance.Set(updateCacheKey, info, updateCacheTTL)
	}
...
func (c *CheckUpdateCmd) fetchLatestRelease(currentVersion string) (UpdateInfo, error) {
	client := &http.Client{
...
func (c *CheckUpdateCmd) displayUpdateInfo(info *UpdateInfo, formatter *output.Formatter) error {
	if info.UpdateAvailable {
		...
		formatter.PrintSuccess(fmt.Sprintf("You're running the latest version (%s)", info.CurrentVersion))
	}
...
func AutoUpdateCheck(version string) {
	...
	if _, ok := cacheInstance.Get(updateCacheKey); ok {
		return
	}
